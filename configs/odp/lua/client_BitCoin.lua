...
gSfAppIdBitCoinClient = 2083
gSfAppIdLiteCoinClient = 2084
...
gAppRegistry = {
	{gSfAppIdBitCoinClient, 0},
    {gSfAppIdLiteCoinClient, 0},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(context.serviceId, appTypeId, context.productId, "", context.appId)
...
function client_init( detectorInstance, configOptions)
    ...
    bitCoinProductId = 263
    liteCoinProductId = 264
     
    bitCoinServiceId = 20154     
    liteCoinServiceId = 20155

	DC.printf ('%s:DetectorValidator(): appTypeId %d, product %d, service %d\n', DetectorPackageInfo.name, appTypeId, bitCoinProductId, bitCoinServiceId)
...
function client_validate()
    ...
    DC.printf ('BitCoin client packetCount %d dir %d, size %d dstPort %d\n', context.packetCount, dir, size, dstPort)

	if (dir == 0 and size > 4) then 
        if (dstPort == 8333) then
            context.serviceId = bitCoinServiceId
            context.productId = bitCoinProductId
            context.appId = gSfAppIdBitCoinClient
            return clientSuccess(context)
        elseif (dstPort == 9333) then
            context.serviceId = liteCoinServiceId
            context.productId = liteCoinProductId
            context.appId = gSfAppIdLiteCoinClient
            return clientSuccess(context)
        end
    end
