...
--]]
--[[
detection_name: Zero VPN
version: 2
description: A free VPN for mobile devices.
--]]
...
gSfAppIdZeroVPN = 4128

gPatterns = {
    hrbt = { '\000\025', 0, gSfAppIdZeroVPN},
    init = { '\114\079\115\070\048\110', 0, gSfAppIdZeroVPN},
}

gFastPatterns = {
    {DC.ipproto.tcp, gPatterns.hrbt},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdZeroVPN);
...
function client_validate()
    ...
    DC.printf ('client Zero VPN: packetCount %d, dir %d, size %d\n', context.packetCount, dir, size);

    -- we expect to see two client packets before any server packets
    if (dir == 0) then

        if ((size == 2) and (gDetector:memcmp(gPatterns.hrbt[1], #gPatterns.hrbt[1], gPatterns.hrbt[2]) == 0)) then
            local rft = FT.getFlowTracker(flowKey)
            if (not rft) then
                DC.printf ('client Zero VPN saw two-byte packet and adding ft for flowKey %s\n', flowKey)
                rft = FT.addFlowTracker(flowKey, {p=1})
                return clientInProcess(context)
            else
                DC.printf ('client Zero VPN more than one two-byte packet in a row\n')
            end
        end

        if ((size == 25) and (gDetector:memcmp(gPatterns.init[1], #gPatterns.init[1], gPatterns.init[2]) == 0)) then
            local rft = FT.getFlowTracker(flowKey)
