...
detection_name: DDS
version: 2
description: Data Distribution Service, communications middleware for data processing applications.
--]]
...
gPatterns = {
    magic = {"RTPS", 0, gSfAppIdDDS},
}

gFastPatterns = {
    {DC.ipproto.udp, gPatterns.magic},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, context.version, gSfAppIdDDS);
...
function client_validate()
    ...
    DC.printf ('client DDS: packetCount %d, dir %d, size %d\n', context.packetCount, dir, size)

    if (dir == 0 and 
        size > 12 and 
        gDetector:memcmp(gPatterns.magic[1], #gPatterns.magic[1], gPatterns.magic[2]) == 0) 
    then
        matched, major_r, minor_r = gDetector:getPcreGroups("(.)(.)", 4)
        if matched then
            major = DC.binaryStringToNumber(major_r, 1)
            minor = DC.binaryStringToNumber(minor_r, 1)
