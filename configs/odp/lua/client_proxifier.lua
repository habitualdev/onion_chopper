...
--]]
--[[
detection_name: Proxifier
version: 4
description: Software that allows a host to route its internet traffic though an external proxy server.
--]]
...
gSfAppIdProxifier = 4136

gPatterns = {
    prox_open = { '\005\001\000', 0, gSfAppIdProxifier},
    prox_cont = { '\005\001\000\001\172\217\008\004\001\187', 0, gSfAppIdProxifier},
    prox_socks4_conn = { '\004\001\001\187', 0, gSfAppIdProxifier},
}

gFastPatterns = {
    {DC.ipproto.tcp, gPatterns.prox_open},
    {DC.ipproto.tcp, gPatterns.prox_socks4_conn},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdProxifier);
...
function client_init( detectorInstance, configOptions)
	...
	appTypeId = 23
	appProductId = 520
    -- note the service here will be SOCKS
	appServiceId = 20019
...
function client_validate()
    ...
    DC.printf ('client proxifier: packetCount %d, dir %d, size %d\n', context.packetCount, dir, size);

    if (dir == 0 and
        size == 9 and
        DC.checkPattern(gDetector, gPatterns.prox_socks4_conn))
    then
        DC.printf ('client proxifier: socks4 connect\n')
        return clientSuccess(context)
    end

    if (dir == 0 and 
        size == 3 and 
        DC.checkPattern(gDetector, gPatterns.prox_open)) 
    then
        DC.printf ('client proxifier: saw first client packet\n')
        -- don't make this check unless we know we have to
    ...
    if (dir == 0 and
        size == 10 and
        DC.checkPattern(gDetector, gPatterns.prox_cont))
    then
        -- don't make this check unless we know we have to
        DC.printf ('client proxifier: got 2nd packet\n')
        local rft = FT.getFlowTracker(flowKey)
